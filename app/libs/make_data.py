...
def generate_data(qty_records: int):
    ...
    dt_retirada = [fake.date_between_dates(date_start=datetime.now(
    ), date_end=datetime.now()+timedelta(days=15)) for i in range(qty_records)]
    cod_reserva = [fake.ean(length=8) for i in range(qty_records)]
    cod_grupo = [np.random.choice(["A", "B", "C", "F", "FX", "GC", "GX"], p=[
                                  0.10, 0.20, 0.25, 0.10, 0.15, 0.10, 0.10]) for i in range(qty_records)]
    ...
    df['dt_devolucao'] = df['dt_retirada'] + \
        timedelta(random.randrange(1, 15, 1))
